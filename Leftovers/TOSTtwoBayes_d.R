...
TOSTtwo<-function(m1,m2,sd1,sd2,n1,n2,low_eqbound_d, high_eqbound_d, alpha, var.equal, prior_dist, effect_prior, se_prior, df_prior){
  ...
  if(var.equal==TRUE) {
    ...
    cohensd<-(m1-m2)/sdpooled
    sed<-(((n1+n2)/(n1*n2))+(cohensd^2/(2*(n1+n2))))
    J<-1-3/(4*(n1+n2-2)-1) #Correction for Hedges g
    hedgesg<-cohensd*J
    seg<-sed*J^2
...
options(scipen=999)
TOSTtwo(m1=5.55,m2=5.22,sd1=0.95,sd2=0.83,n1=95,n2=89,low_eqbound_d=-0.43,high_eqbound_d=0.43, var.equal=TRUE, prior_dist = "normal", effect_prior = 0.2)

TOSTtwo(m1=5.55,m2=5.22,sd1=0.95,sd2=0.83,n1=95,n2=89,low_eqbound_d=-0.43,high_eqbound_d=0.43, var.equal=TRUE, prior_dist = "halfnormal", effect_prior = 0.2, se_prior = 0.3, df_prior = 81)
