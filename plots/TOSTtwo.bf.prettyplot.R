TOSTtwo.bf.prettyplot<-function(m1,m2,sd1,sd2,n1,n2,low_eqbound_d, high_eqbound_d, alpha, var.equal, prior_dist, effect_prior, se_prior, df_prior, uniform_lower_bound, uniform_upper_bound)
  {
  if(missing(alpha)) {
    alpha<-0.05
  }
  ...
  if(bayes==TRUE){
    ...
    if(bayes==TRUE){
      ...
      plot(theta, likelihood_alt, type = "l",
           ylim = c(0, 1),
           xlim=c(min(LL90,low_eqbound)-max(UL90-LL90, high_eqbound-low_eqbound)/5, max(UL90,high_eqbound)+max(UL90-LL90, high_eqbound-low_eqbound)/5),     col = "grey74", lwd = 4, lty = 3, axes = F, xlab = NA, ylab = NA)
      axis(side = 4)
